...
def get_channel(bs_loc, ue_pos, scatterers, band):
    if conf.dimensions == DimensionType.Three.name:
        bs_ue_channel = get_3d_channel(np.array(bs_loc), ue_pos, scatterers, band)
    else:
        bs_ue_channel = get_2d_channel(np.array(bs_loc), ue_pos, scatterers, band)
