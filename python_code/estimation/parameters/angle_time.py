...
class AngleTimeEstimator:
    ...
    def __init__(self, bands: List[Band]):
        ...
        if self.multi_band:
            if torch.cuda.is_available():
                ...
                self.mats = [[self.angle_estimator._angle_options[i],
                              self.time_estimator._time_options[i]]
                             for i in range(len(bands))]
    ...
    def estimate(self, y: Union[np.ndarray, List[np.ndarray]]) -> Estimation:
        ...
        spectrum_power = self._spectrum[self.indices[:, 0], self.indices[:, 1]]
        AOA = self.angle_estimator.aoa_angles_dict[self._aoa_indices]
        if self.multi_band:
            TOA = self.time_estimator.times_dict[self.algorithm.k][self._toa_indices]
        else:
            TOA = self.time_estimator.times_dict[self._toa_indices]
        estimator = Estimation(AOA=AOA, TOA=TOA, POWER=spectrum_power)
        return estimator
